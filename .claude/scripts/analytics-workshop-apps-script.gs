/**
 * Analytics workshop submissions — Apps Script web app.
 *
 * Separate deployment from the original pace workshop on purpose
 * (Shep's call, 2026-09-15): its own sheet, its own Drive folder,
 * its own /exec URL. Same code otherwise.
 *
 * Setup (one time):
 * 1. Create a NEW Google Sheet named "rhoe-analytics-workshop-submissions".
 * 2. In that sheet: Extensions → Apps Script, paste this whole file in.
 * 3. Deploy → New deployment → Web app:
 *      - Execute as: Me
 *      - Who has access: Anyone
 * 4. Copy the /exec URL into ANALYTICS_ENDPOINT at the top of
 ...
 * (?exercise=experimentation filters). Emails are NEVER included in the
 * doGet output — the endpoint is public.
 */

var FOLDER_NAME = 'analytics-workshop-submissions';
